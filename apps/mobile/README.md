...
2. **Live monitor (optional):** with **Start monitor**, the app keeps `CameraView` in **`video`** mode and runs a **continuous async loop**: snapshot JPEG (**~0.38 quality**) → **`POST /api/perception/detect`** → **green** overlays + labels for each detection (no fixed interval; round-trip ms is shown in the HUD). High client HUD score can trigger a **debounced** full pipeline (same steps 3–6 as a single burst). Short **expo-speech** hints use a separate cooldown and are not a substitute for `/api/voice/alert` on saved events.
